...
-(void)initDB
{
        ...
        databaseFullPath = [AppConfiguration dbPath];
            ...
            if (!queryExecutable){
                ...
                while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
                    ...
                    for (int i=0; i<totalColumns; i++){
                        ...
                        [arrColumnNames addObject:[NSString stringWithUTF8String:dColumnNameAsChars]];
...
-(BOOL)hasData
{
    if(currentIndex  < arrResults.count){
        return YES;
    }
    currentIndex = 0;
    return NO;
...
{
    if(currentIndex  < arrResults.count){
        NSMutableDictionary * valuesMap = [arrResults objectAtIndex:currentIndex++];
        return valuesMap;
    }
    currentIndex = 0;
    return nil;
...
{
    id retValue = nil;
    for(NSMutableDictionary * valuesMap in arrResults){
        retValue = [valuesMap objectForKey:name];
        if(retValue != nil){
            return retValue;
        }
    }
